...
namespace Mindtree.WordProcessingAPI1.WordProcessor
{

    /// <summary>
    /// This class will hold the info regarding sub heading of the Survey Documents
    /// </summary>
    ...
    /// <summary>
    /// This enum will be using to setting custom properties of the Documents
    /// </summary>
    ...
    /// <summary>
    /// This class is used to manupulate the questions 
    /// </summary>
    ...
    /// <summary>
    /// This class has all the method for process the document 
    /// </summary>
    public class WordProcessor
    {


        private string docProtectionPasssword;
        public WordProcessor()
        {
            if (!(string.IsNullOrEmpty(ConfigurationManager.AppSettings["wordDocProtectionPassword"])))
            {

                docProtectionPasssword = ConfigurationManager.AppSettings["wordDocProtectionPassword"].ToString();
            }
            else
            {
                throw new ArgumentNullException("Value for the appsetting  wordDocProtectionPassword is empty");
            }
        }
        //Temp Physical path for the documents
        //string surveyInput = @"E:\Survey Opinion Latest\Input_OutPut_Files\IO\QuestionTemplate.docx";
        //string surveyOutput = @"E:\Survey Opinion Latest\Input_OutPut_Files\IO\AnswerTemplate.docx";

        #region Template Processing API


        /// <summary>
        /// This method is used to create a document template based on the number of SubHeadingNo and number of questions
        /// </summary>
        /// <param name="SubHeadingNo"></param>
        ...
        /// <summary>
        /// THis method is used to add header to a given  document template
        /// </summary>
        ...
        public byte[] GetOpinionTemplateWithHeader(byte[] NewTemplateDocument, DocumentMetaData metaData)
        {
            try
            {
                ...
                {
                    string survey = metaData.OpinionName; string DescriptionContent = metaData.OpinionDescription;
                    byte[] finalqTemplate;
                    using (MemoryStream streamQuestionTemplate = new MemoryStream())
                    {
                        ...
                        using (WordprocessingDocument wordDocumentInput = WordprocessingDocument.Open(streamQuestionTemplate, true))
                        {
                            ...
                            //Survey Opinion Names Start
                            List<SdtBlock> SurveyBlock = wordDocumentInput.MainDocumentPart.Document.Body.Descendants<SdtBlock>().Where(r => r.SdtProperties.GetFirstChild<Tag>().Val.Value == "survey").ToList();
                            if (SurveyBlock.Count == 0)
                            {
                                Run runsurveyBlock = new Run();
                                RunProperties runPropssurveyBlock = new RunProperties();
                                Bold boldsurveyBlock = new Bold();
                                Underline ulsurveyBlock = new Underline() { Val = DocumentFormat.OpenXml.Wordprocessing.UnderlineValues.Single };

                                runPropssurveyBlock.Append(boldsurveyBlock);
                                runPropssurveyBlock.Append(ulsurveyBlock);
                                RunFonts runFontsurveyBlock = new RunFonts();           // Create font
                                runFontsurveyBlock.Ascii = "Times New Roman";
                                runPropssurveyBlock.Append(runFontsurveyBlock);
                                FontSize fontSizesurveyBlock = new FontSize() { Val = "30" };
                                runPropssurveyBlock.Append(fontSizesurveyBlock);
                                runsurveyBlock.AppendChild(new RunProperties(runPropssurveyBlock));
                                runsurveyBlock.AppendChild(new Text(survey));
                                Paragraph paragraphsurveyBlock = new Paragraph(runsurveyBlock);
                                SdtProperties sdtprsurveyBlock = new SdtProperties(
                                        new SdtAlias { Val = "survey" },
                                        new Tag { Val = "survey" });
                                SdtContentBlock sdtCBlocksurveyBlock = new SdtContentBlock(paragraphsurveyBlock);
                                SdtBlock sdtBlocksurveyBlock = new SdtBlock(sdtprsurveyBlock, sdtCBlocksurveyBlock);
                                firstSection[0].InsertBeforeSelf<SdtBlock>(sdtBlocksurveyBlock);
                                //Survey Opinion Names End
                            }
                            //--
                        ...
                        byte[] qTemplateOpinionName = WDSetCustomProperty(qTemplateMetaDataOpinionDocVersion, "OpinionName", survey, PropertyTypes.Text);
        ...
        /// <summary>
        /// This method will make a answer template where the company can put the answer
        /// </summary>
        /// <param name="DocumentQuestionTemplate"></param>
        /// <param name="metaData"></param>
        /// <returns></returns>
        public byte[] GetDistributableDocument(byte[] DocumentQuestionTemplate, DocumentMetaData metaData)
        {
            ...
            //Copying the "Survey Opinion Name" and "Survey Opinion description" from "Question Template"
            //to avoid mismatch
            ...
            byte[] fileBytesAnswerTemplate = CreatesurveyOutput(DocumentQuestionTemplate, DocumentAnswerTemplate, metaData, false);
            ...
            byte[] AnswerTemplatewithCompanyid = WDSetCustomProperty(AnswerTemplatewithOpinionDocVersion, "CompanyId", metaData.CompanyId, PropertyTypes.NumberInteger);
            byte[] AnswerTemplatewithDocType = WDSetCustomProperty(AnswerTemplatewithCompanyid, "DocumentType", DocumentType.ANSWER_TEMPLATE.ToString(), PropertyTypes.Text);
        ...
        /// <summary>
        /// This method will read all the meta data from the document
        /// </summary>
        /// <param name="documentAnsTemplate"></param>
        /// <returns></returns>
        public DocumentMetaData GetDocumentMetadata(byte[] documentAnsTemplate)
        {
            ...
            try
            {
                ...
                string Companyid = metaDataDictionary.FirstOrDefault(x => x.Key == "CompanyId").Value;
                ...
                using (MemoryStream streamAnsTemplate = new MemoryStream())
                {
                    ...
                    using (WordprocessingDocument outPutDoc = WordprocessingDocument.Open(streamAnsTemplate, true))
                    {
                        ...
                        _metaDataRecords.CompanyName = CompanyName;
                        ...
                        _metaDataRecords.CompanyId = Convert.ToInt32(Companyid);
        ...
        /// <summary>
        /// This method will merge the Aswer template and the question template if there is any update in either of them
        /// </summary>
        /// <param name="NewTemplateDocument"></param>
        /// <param name="SubRecordDocumentWithAnswers"></param>
        /// <returns></returns>
        public byte[] GetMergedDocumentWithAnswers(byte[] NewTemplateDocument, byte[] SubRecordDocumentWithAnswers)
        {
            ...
            if (docPreparedforExecution != "true")
            {
                ...
                if (Convert.ToInt32(OpinionDocumentVersionQuesTemplate) != Convert.ToInt32(OpinionDocumentVersionAnsTemplate))
                {
                    ...
                    byte[] fileBytesAnswerTemplate = CreatesurveyOutput(NewTemplateDocument, SubRecordDocumentWithAnswers, metaData, true);
        ...
        /// <summary>
        // This method will  make the question/Answers noneditable if they are mark for completion
        /// </summary>
        /// <param name="surveyOutput"></param>
        /// <returns></returns>
        public byte[] MarkCompletionAnswerTemplate(byte[] surveyOutput)
        {
            ...
            {
                streamAnswerTemplate.Write(surveyOutput, 0, surveyOutput.Length);
        ...
        /// <summary>
        /// This method will delete the question from the answer document which are marked for deletion in the question document
        /// </summary>
        /// <param name="surveyInput"></param>
        /// <param name="surveyOutput"></param>
        /// <returns></returns>
        public byte[] CreateDeletedQuestionOutputAnswerTemplate(byte[] surveyInput, byte[] surveyOutput)
        {
            WordProcessor objWordProcessor = new WordProcessor();
            byte[] outputDocument;
            using (MemoryStream streamQuestionTemplate = new MemoryStream())
            {
                streamQuestionTemplate.Write(surveyInput, 0, surveyInput.Length);
        ...
        /// <summary>
        /// This method will  make the question from the quetion document non editable which are marked for deletion
        /// </summary>
        /// <param name="surveyInput"></param>
        /// <returns></returns>
        public byte[] CreateDeletedQuestionOutputOpinionTemplate(byte[] surveyInput)
        {
            WordProcessor objWordProcessor = new WordProcessor();
            byte[] inputDocument;
            using (MemoryStream streamQuestionTemplate = new MemoryStream())
        ...
        /// <summary>
        /// Upon reciving the answer document from the company, this method will convert the document for survey users
        /// review purpose by adding check boxes and giving required permissions
        /// </summary>
        /// <param name="DocumentFromCompany"></param>
        /// <returns></returns>
        public byte[] ConvertAnswerTemplateForSurveyUser(byte[] DocumentFromCompany)
        {
            //byte[] documentForSurveyUser = new byte[100];
            //To Do: Add perm start and perm end needs to be added before and after the check boxes in this document. 
            //This will allow the Survey users to check the complete box in addition to editing answers. 
            //The Survey user should not be able to edit anything else.
            WordProcessor objWordProcessor = new WordProcessor();
            byte[] AnswerTemplateForSurveyUser;
            try
            {
                using (MemoryStream streamAnswerTemplate = new MemoryStream())
                {
                    streamAnswerTemplate.Write(DocumentFromCompany, 0, DocumentFromCompany.Length);
                    using (WordprocessingDocument ansDoc = WordprocessingDocument.Open(streamAnswerTemplate, true))
                    {
                    ...
                    AnswerTemplateForSurveyUser = objWordProcessor.ReadToEnd(streamAnswerTemplate);

                }

                using (MemoryStream streamAnswerTemplate = new MemoryStream())
                {
                    streamAnswerTemplate.Write(AnswerTemplateForSurveyUser, 0, AnswerTemplateForSurveyUser.Length);
                    using (WordprocessingDocument ansDoc = WordprocessingDocument.Open(streamAnswerTemplate, true))
                    {
                        //
                        ...
                        if (!(trackRevisionSetting.Any()))
                        {
                            ...
                            return OutputDocEnableTracking(AnswerTemplateForSurveyUser);
                        }
                        else
                            return AnswerTemplateForSurveyUser;
                    }
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Once all answers are marked completed, this  method will add Submitted by and Submitted fields but the value should be empty and editable
        /// </summary>
        /// <param name="InputDocumentForExecution"></param>
        /// <returns></returns>
                ...
                using (WordprocessingDocument outputDoc = WordprocessingDocument.Open(streamAnswerTemplate, true))
                {
                    ...
                    foreach (OpenXmlElement sdtoutputDoc in blocksoutputDoc)
                    {
                        ...
                        if (sdtoutputDoc.GetType().Name == "Table")
                        {
                            ...
                            if (ContentTagName == "surveyNameKeyTag")
                            {
                                foreach (string Data in ExecutionData)
                                {
                                    ...
                                    TableRow trsurveyCover = new TableRow();
                                    TableCell tcsurveyCover = new TableCell();
                                    tcsurveyCover.Append(sdtBlockparagraphMetaData);
                                    TableCell tcsurveyNameCover = new TableCell();
                                    tcsurveyNameCover.Append(sdtBlockparagraphMetaDataValue);
                                    trsurveyCover.Append(tcsurveyCover);
                                    trsurveyCover.Append(tcsurveyNameCover);
                                    sdtoutputDoc.Append(trsurveyCover);
        ...
        /// <summary>
        /// this method  make the entire document non editable once the survey is complete
        /// </summary>
        /// <param name="InputExecuteDocument"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// This method is used  to accept the revisions based on the the Track Changes
        /// </summary>
        ...
        /// <summary>
        /// This method is used for creating Comparison Report from the input answer documents provided
        /// </summary>
        /// <param name="CompanysDocument"></param>
        /// <param name="questionNumbers"></param>
        /// <returns></returns>
        public byte[] CreateComparisonReportDocument(Dictionary<string, byte[]> CompanysDocument, List<string> questionNumbers)
        {
            ...
            using (MemoryStream streamOutputDoc = new MemoryStream())
            {
                ...
                CreateComparisonReportCoverPage(streamOutputDoc, CompanysDocument);
                //
                ...
                Dictionary<string, byte[]> updatedCompanysDocument = new Dictionary<string, byte[]>();
                foreach (KeyValuePair<string, byte[]> firmDoc in CompanysDocument)
                {
                    using (MemoryStream streamInputDoc = new MemoryStream())
                    {
                        ...
                        using (WordprocessingDocument inputDoc = WordprocessingDocument.Open(streamInputDoc, true))
                        {
                            ...
                            inputDoc.MainDocumentPart.Document.Save();
                            inputDoc.Dispose();
                            updatedCompanysDocument.Add(firmDoc.Key, ReadToEnd(streamInputDoc));
                        }
                    }
                }
                ...
                ApplyHeader(outputDoc, "... Survey Opinion Comparison Report", false, false);
                ApplyFooter(outputDoc);
                //Header Footer Section Ends
                foreach (var item in QuestionSectionTag)
                {
                    foreach (KeyValuePair<string, byte[]> firmDoc in updatedCompanysDocument)
                    {
                        using (MemoryStream streamInputDoc = new MemoryStream())
                        {
                            streamInputDoc.Write(firmDoc.Value, 0, firmDoc.Value.Length);
                            ...
                            Dictionary<string, string> customPropsDictionary = WDGetCustomProperty(firmDoc.Value, "surveyName", "surveyDesc", "CompanyName", "JurisdictionName");
                                ...
                                foreach (OpenXmlElement sdtinputDoc in blocksinputDoc)
                                {
                                    ...
                                    if (sdtinputDoc.GetType().Name == "SdtBlock")
                                    {
                                        ...
                                        if (sdtinputDoc.Descendants<Tag>().FirstOrDefault().Val.Value == answerTag && sdtinputDoc.Descendants<Tag>().FirstOrDefault() != null)
                                        {
                                            ...
                                            //Company
                                            Run runCompany = new Run();

                                            RunProperties runPropsCompany = new RunProperties();

                                            RunFonts runFontCompany = new RunFonts();           // Create font
                                            runFontCompany.Ascii = "Times New Roman";
                                            runPropsCompany.Append(runFontCompany);

                                            FontSize fontSizeCompany = new FontSize() { Val = "27" };
                                            runPropsCompany.Append(fontSizeCompany);

                                            Bold boldCompany = new Bold();
                                            runPropsCompany.Append(boldCompany);

                                            runCompany.AppendChild(new RunProperties(runPropsCompany));
                                            runCompany.AppendChild(new Text(firmDoc.Key + "-" + juridictionName));
                                            Paragraph paragraphCompany = new Paragraph(runCompany);
                                            SdtProperties sdtPrCompany = new SdtProperties(
                                                    ...
                                                    new Lock { Val = LockingValues.SdtLocked });
                                            SdtContentBlock sdtCBlockCompany = new SdtContentBlock(paragraphCompany);
                                            SdtBlock sdtBlockCompany = new SdtBlock(sdtPrCompany, sdtCBlockCompany);
                                            //
                                            ...
                                            if (sectionExistList.Count == 0)
                                            {
                                                bodyoutputDoc.Append(sdtBlockCompany);
                                                bodyoutputDoc.Append(sdtBlockAnswerHeader);
                                                bodyoutputDoc.Append((OpenXmlElement)sdtinputDoc.Clone());
                                            }
                                            else
                                            {
                                                sectionExistList[0].InsertBeforeSelf<OpenXmlElement>(sdtBlockCompany);
        ...
        /// <summary>
        ///  This method is used to revert back the Execute Document to Distributed Document
        /// </summary>
        /// <param name="ExecutedDocument"></param>
        /// <returns></returns>
        public byte[] CreateRevertedExecuteDocument(byte[] ExecutedDocument)
        {
            ...
            using (MemoryStream streamAnswerTemplate = new MemoryStream())
            {
                ...
                OutputDocumentForExecution = ConvertAnswerTemplateForSurveyUser(OutputDocumentForExecution);
            }
            return OutputDocumentForExecution;
        ...
        /// <summary>
        /// This method is used to modify the cover PAGE
        /// </summary>
        /// <param name="surveyOutput"></param>
        /// <param name="metaData"></param>
        /// <returns></returns>
        public byte[] ModifyCoverPage(byte[] surveyOutput, DocumentMetaData metaData)
        {
            WordProcessor objWordProc = new WordProcessor();
            byte[] outPutDocument;
            Dictionary<string, string> metaDataDictionary = WDGetCustomProperty(surveyOutput,
                                                                                "docPreparedforExecution");
            ...
            using (MemoryStream streamAnswerTemplate = new MemoryStream())
            {
                ...
                ApplyHeader(outputDoc, (metaData.CompanyName + "/" + metaData.OpinionName), true, !isDocumentPreparedForExecution);
                    ...
                    if (sdtoutputDoc.GetType().Name == "Table")
                    {
                        foreach (TableRow tr in sdtoutputDoc.Elements<TableRow>())
                        {
                            foreach (TableCell tc in tr.Elements<TableCell>())
                            {
                                ...
                                if (tagString == "surveyNameValueTag" && metaData.OpinionName != "" && metaData.OpinionName != null)
                                {
                                    int i = 0;
                                    //[0] position has the title "Survey Opinion Name :", so starting from [1]
                                ...
                                if (tagString == "FirmNameValueTag" && metaData.CompanyName != "" && metaData.CompanyName != null)
                                {
                                    int i = 0;
                                    ...
                                    existingText.Text = metaData.CompanyName;
                                }
                ...
                //ApplyHeader(outputDoc, (metaData.CompanyName + "/" + metaData.OpinionName));
                outputDoc.MainDocumentPart.Document.Save();
                outputDoc.Dispose();
        ...
        /// <summary>
        /// This method is used  to get List of all the SubheadingName and corresponding question numbers present in the document
        /// </summary>
        ...
        private List<String> GetQuestionTagList(Dictionary<string, byte[]> CompanysDocument, List<string> questionNumbers)
        {
            List<string> QuestionSectionTag = new List<string>();
            foreach (KeyValuePair<string, byte[]> firmDoc in CompanysDocument)
            {
        ...
        private void CreateBlankOutputDoc(Stream file, bool isThisOutputFile)
        {
            using (WordprocessingDocument myDoc = WordprocessingDocument.Create(file, WordprocessingDocumentType.Document))
            {
                ...
                _userPermissions.ApplyDocumentProtection(myDoc, docProtectionPasssword);
        ...
        //private byte[] CreatesurveyOutput(byte[] surveyInput, byte[] surveyOutput, string Jurisdictionname, string CompanyName, bool isMargeCalled)
        private byte[] CreatesurveyOutput(byte[] surveyInput, byte[] surveyOutput, DocumentMetaData metaData, bool isMargeCalled)
        {
            WordProcessor objWordProcessor = new WordProcessor();
            byte[] mergedDocument;
            int ansPermId = 0;
            using (MemoryStream streamQuestionTemplate = new MemoryStream())
            {
                ...
                using (MemoryStream streamAnswerTemplate = new MemoryStream())
                {
                    ...
                    using (WordprocessingDocument inputDoc = WordprocessingDocument.Open(streamQuestionTemplate, true))
                    {
                        ...
                        if (isMargeCalled == false)
                        {
                            //ApplyHeader(outputDoc, metaData.CompanyName);
                            ApplyHeader(outputDoc, (metaData.CompanyName + "/" + metaData.OpinionName), false, false);
                            ApplyFooter(outputDoc);
                        }
                        ...
                        foreach (OpenXmlElement sdtinputDoc in blocksinputDoc)
                        {
                            if (sdtinputDoc.Descendants<Tag>().FirstOrDefault() != null)
                            {
                                if (sdtinputDoc.Descendants<Tag>().FirstOrDefault().Val.Value == "survey" || sdtinputDoc.Descendants<Tag>().FirstOrDefault().Val.Value == "DescriptionTag" || sdtinputDoc.Descendants<Tag>().FirstOrDefault().Val.Value == "DescriptionContentTag")
                                {
                                }
                                else
                                {
                                    ...
                                    if (isContentBlockExist == false)
                                    {
                                        ...
                                        if (sdtinputDoc.GetType().Name == "Table")
                                        {
                                            ...
                                            Underline ulsurveyBlock = new Underline() { Val = DocumentFormat.OpenXml.Wordprocessing.UnderlineValues.Single };
                                            runPropsQuestionNo.Append(ulsurveyBlock);
                    ...
                    byte[] outPutDocumentwithProps1 = WDSetCustomProperty(outPutDocument, "surveyName", metaData.OpinionName, PropertyTypes.Text);
                    byte[] outPutDocumentwithProps2 = WDSetCustomProperty(outPutDocumentwithProps1, "surveyDesc", metaData.OpinionDescription, PropertyTypes.Text);
                    byte[] outPutDocumentwithProps3 = WDSetCustomProperty(outPutDocumentwithProps2, "CompanyName", metaData.CompanyName, PropertyTypes.Text);
        ...
        private void CreateOutputDocCoverPage(MemoryStream surveyOutput, DocumentMetaData metaData)
        {
            ...
            metaDataDictionary.Add("Survey Opinion Name", metaData.OpinionName);
            ...
            metaDataDictionary.Add("Firm Name", metaData.CompanyName);
            //--
            using (WordprocessingDocument outputDoc = WordprocessingDocument.Open(surveyOutput, true))
            {
                ...
                TableWidth tableWidthsurveyCover = new TableWidth() { Width = "5000", Type = TableWidthUnitValues.Pct };
                tableProp.Append(tableWidthsurveyCover);
                ...
                foreach (string metaDataKey in metaDataDictionary.Keys)
                {
                    ...
                    TableRow trsurveyCover = new TableRow();
                    TableCell tcsurveyCover = new TableCell();
                    TableCellProperties tcsurveyCoverProps = new TableCellProperties();
                    TableCellWidth tcWidthtcsurveyCover = new TableCellWidth() { Width = "2000", Type = TableWidthUnitValues.Pct };
                    tcsurveyCoverProps.Append(tcWidthtcsurveyCover);
                    tcsurveyCover.Append(tcsurveyCoverProps);
                    tcsurveyCover.Append(sdtBlockparagraphMetaData);
                    TableCell tcsurveyNameCover = new TableCell();
                    tcsurveyNameCover.Append(sdtBlockparagraphMetaDataValue);
                    trsurveyCover.Append(tcsurveyCover);
                    trsurveyCover.Append(tcsurveyNameCover);
                    TblMetaDataInfo.Append(trsurveyCover);
            ...
            using (WordprocessingDocument outputDocbreak = WordprocessingDocument.Open(surveyOutput, true))
            {
        ...
        private void CreateComparisonReportCoverPage(MemoryStream surveyOutput, Dictionary<string, byte[]> CompanysDocument)
        {
            Dictionary<string, string> CompanyDictionary = new Dictionary<string, string>();
            CompanyDictionary.Add("Law Firms selected for comparison:", "");
            CompanyDictionary.Add("Law Firm", "Jurisdiction");
            Dictionary<string, string> metaDataDictionary = new Dictionary<string, string>();
            string surveyName = "";
            string surveyDesc = "";
            try
            {
                foreach (KeyValuePair<string, byte[]> firmDoc in CompanysDocument)
                {
                    ...
                    surveyName = customPropsDictionary.FirstOrDefault(x => x.Key == "surveyName").Value;
                    surveyDesc = customPropsDictionary.FirstOrDefault(x => x.Key == "surveyDesc").Value;
                    string CompanyName = customPropsDictionary.FirstOrDefault(x => x.Key == "CompanyName").Value;
                    string JurisdictionName = customPropsDictionary.FirstOrDefault(x => x.Key == "JurisdictionName").Value;
                    CompanyDictionary.Add(CompanyName, JurisdictionName);
                }
                //--
                string dateSubmitted = System.DateTime.Today.ToLongDateString();//.ToString();
                metaDataDictionary.Add("Report Title:", "... Survey Opinion Comparison Report");
                metaDataDictionary.Add("Date Created:", dateSubmitted);
                metaDataDictionary.Add("Report Purpose:", "Comparison multiple Law Firm answers for a single question from a ... Survey Opinion.");
                metaDataDictionary.Add("Survey Opinion:", surveyName);
                metaDataDictionary.Add("", surveyDesc);
                ...
                using (WordprocessingDocument outputDoc = WordprocessingDocument.Open(surveyOutput, true))
                {
                    ...
                    foreach (string metaDataKey in metaDataDictionary.Keys)
                    {
                        ...
                        TableCellProperties tcsurveyCoverProps = new TableCellProperties();
                        TableCellWidth tcWidthtcsurveyCover = new TableCellWidth() { Width = "1500", Type = TableWidthUnitValues.Pct };
                        tcsurveyCoverProps.Append(tcWidthtcsurveyCover);
                        tcsurveyCover.Append(tcsurveyCoverProps);

                        tcsurveyCover.Append(sdtBlockparagraphMetaData);
                        TableProperties tablePropsurveyCover = new TableProperties();
                        TableWidth tableWidthsurveyCover = new TableWidth() { Width = "5000", Type = TableWidthUnitValues.Pct };
                        tablePropsurveyCover.Append(tableWidthsurveyCover);
                        TableCell tcsurveyNameCover = new TableCell();
                        tcsurveyNameCover.Append(sdtBlockparagraphMetaDataValue);
                        trsurveyCover.Append(tcsurveyCover);
                        TblGenericDataInfo.AppendChild(tablePropsurveyCover);
                        trsurveyCover.Append(tcsurveyNameCover);
                        TblGenericDataInfo.Append(trsurveyCover);


                    }
                    #endregion
                    #region Given Companys Details
                    Table TblCompanysDataInfo = new Table();
                    // Set the style and width for the table.
                    TableProperties tableCompanysProp = new TableProperties();


                    foreach (string CompanyDataKey in CompanyDictionary.Keys)
                    {
                        //--Entry for the Company Data Key starts
                        Run RunCompanyData = new Run();
                        RunProperties runPropsCompanyData = new RunProperties();
                        Bold boldCompanyData = new Bold();
                        FontSize fontSizeCompanyData = new FontSize() { Val = "28" };
                        if (CompanyDataKey == "Law Firms selected for comparison:" || CompanyDataKey == "Law Firm")
                            runPropsCompanyData.Append(boldCompanyData);
                        runPropsCompanyData.Append(fontSizeCompanyData);

                        RunFonts runFontCompanyData = new RunFonts();           // Create font
                        runFontCompanyData.Ascii = "Times New Roman";
                        runPropsCompanyData.Append(runFontCompanyData);

                        RunCompanyData.AppendChild(new RunProperties(runPropsCompanyData));
                        RunCompanyData.AppendChild(new Text(CompanyDataKey));
                        Paragraph paragraphCompanyData = new Paragraph(RunCompanyData);
                        SdtProperties sdtPrparagraphCompanyData = new SdtProperties(
                                       new SdtAlias { Val = CompanyDataKey + "KeyTitle" },
                                       new Tag { Val = CompanyDataKey + "KeyTag" },
                                       new Lock { Val = LockingValues.ContentLocked },
                                       new Lock { Val = LockingValues.SdtLocked });
                        SdtContentBlock sdtCBlockparagraphCompanyData = new SdtContentBlock(paragraphCompanyData);
                        SdtBlock sdtBlockparagraphCompanyData = new SdtBlock(sdtPrparagraphCompanyData, sdtCBlockparagraphCompanyData);
                        //-- Entry for the Titles ends
                        string CompanyDataValue = CompanyDictionary.FirstOrDefault(x => x.Key == CompanyDataKey).Value;
                        //--Entry for Company Data Values starts
                        Run RunCompanyDataValue = new Run();
                        RunProperties runPropsCompanyDataValue = new RunProperties();

                        Bold boldCompanyDataValue = new Bold();
                        if (CompanyDataValue == "Jurisdiction")
                            runPropsCompanyDataValue.Append(boldCompanyDataValue);

                        FontSize fontSizeCompanyDataValue = new FontSize() { Val = "28" };
                        runPropsCompanyDataValue.Append(fontSizeCompanyDataValue);

                        RunFonts runFontCompanyDataValue = new RunFonts();           // Create font
                        runFontCompanyDataValue.Ascii = "Times New Roman";
                        runPropsCompanyDataValue.Append(runFontCompanyDataValue);

                        RunCompanyDataValue.AppendChild(new RunProperties(runPropsCompanyDataValue));
                        RunCompanyDataValue.AppendChild(new Text(CompanyDataValue));
                        Paragraph paragraphCompanyDataValue = new Paragraph(RunCompanyDataValue);
                        SdtProperties sdtPrparagraphCompanyDataValue = new SdtProperties(
                                       new SdtAlias { Val = CompanyDataKey + "ValueTitle" },
                                       new Tag { Val = CompanyDataKey + "ValueTag" },
                                       new Lock { Val = LockingValues.ContentLocked },
                                       new Lock { Val = LockingValues.SdtLocked });
                        SdtContentBlock sdtCBlockparagraphCompanyDataValue = new SdtContentBlock(paragraphCompanyDataValue);
                        SdtBlock sdtBlockparagraphCompanyDataValue = new SdtBlock(sdtPrparagraphCompanyDataValue, sdtCBlockparagraphCompanyDataValue);
                        //-- Entry for Meta Data Values
                        ...
                        TableCellProperties tcsurveyCoverProps = new TableCellProperties();
                        TableCellWidth tcWidthtcsurveyCover = new TableCellWidth() { Width = "2500", Type = TableWidthUnitValues.Pct };
                        tcsurveyCoverProps.Append(tcWidthtcsurveyCover);
                        tcsurveyCover.Append(tcsurveyCoverProps);

                        tcsurveyCover.Append(sdtBlockparagraphCompanyData);
                        TableProperties tablePropsurveyCover = new TableProperties();
                        TableWidth tableWidthsurveyCover = new TableWidth() { Width = "5000", Type = TableWidthUnitValues.Pct };
                        tablePropsurveyCover.Append(tableWidthsurveyCover);
                        TableCell tcsurveyNameCover = new TableCell();
                        tcsurveyNameCover.Append(sdtBlockparagraphCompanyDataValue);
                        trsurveyCover.Append(tcsurveyCover);
                        TblCompanysDataInfo.AppendChild(tablePropsurveyCover);
                        trsurveyCover.Append(tcsurveyNameCover);
                        TblCompanysDataInfo.Append(trsurveyCover);
                    ...
                    bodyoutputDoc.Append(TblCompanysDataInfo);
                    //--Cover Page Entry ends
    ...
    /// <summary>
    /// This class used to deal with the track changes  
    /// </summary>
    public static class OpenXMLExtensions
    {
        /// <summary>
        /// This method used for accepting revisions of the track changes 
        /// </summary>
